...
for tt=1:length(t)
    x_FM(tt) = exp(j*2*pi*(f_c+CFO)*t(tt) + j*2*pi*Df*sum(m(1:tt))*Ts);
    %x_FM2(tt) = (exp(j*2*pi*(f_c+CFO)*t(tt) + j*2*pi*Df*sum(m(1:tt))*Ts) + exp(-j*2*pi*(f_c+CFO)*t(tt) - j*2*pi*Df*sum(m(1:tt))*Ts))/2;
end
...
if display_figures
    ...
    title('Q-phase sampled symbols T');
    % plot imaginary part of sampled symbols r_k
    subplot(2, 1, 2);
    stem(imag(r_k), 'r');
    title('Q-phase sampled symbols R');
