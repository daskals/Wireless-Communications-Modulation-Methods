...
%% Tag parameters
    chanParams.Tsymbol = 500e-6; %Tsymbol of the tag 
    chanParams.HammingCode=0;   %Hamming code enabled
    
%% Noise Param
   chanParams.noiseSNR=40;
   chanParams.P_TX = 20; 
   
%% Modulation Selection Param
   chanParams.Modtype=1; % 0 for 2PAM ----1 for 4 PAM
...
%% Initiallization
 ...
 %% PacketsSimulation
    ...
    snrR=[-20 -15 -10 -5 0 5 10 15 20 25 30 35 40];
    mo=[0 1];
...
[x_FM]= FM_carrier_channel(total_packet_duration, chanParams);
 ...
 t = 0:chanParams.Ts:t_sampling-chanParams.Ts;
  ...
  for (i=1:1: length(snrR)) 
    chanParams.noiseSNR= snrR(i);
   for (steps=1:1:N)
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        PER(i)=error_packets/(correct_packets+error_packets);
        BER(i)=Bit_errors_sum/((correct_packets+error_packets)*fixedpacketdata_len);
